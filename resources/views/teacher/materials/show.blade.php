...
<div class="breadcrumb mb-4">
    <a href="{{ route('teacher.courses.show', $material->course_id) }}">{{ __('app.materials') }}</a> / <span>{{ $material->title }}</span>
</div>
...
<div class="card mb-6">
    ...
    <div class="card-body">
        ...
        <div style="margin-top: 20px; padding: 20px; background: var(--bg); border: 1px solid var(--border); border-radius: 8px;">
            ...
            <p class="mb-4">{{ __('app.download') }} file materi pendukung:</p>
...
<div class="card mb-6">
    ...
    <div class="card-body">
        <div class="comments-section" style="max-height: 500px; overflow-y: auto; padding-right: 10px;">
            ...
            <div style="padding: 15px; border: 1px solid var(--border); background: {{ $comment->is_pinned ? '#fffbeb' : '#fff' }}; border-radius: 8px; margin-bottom: 12px; position: relative;" x-data="{ replyOpen: false }">
                ...
                <div class="flex gap-3 mb-3">
                    ...
                    <div class="flex-1">
                        <div class="flex flex-between">
                            <span class="font-semibold" style="color: var(--primary);">{{ $comment->user->name }} <span class="badge badge-secondary" style="font-size:0.65rem; margin-left:4px;">{{ ucfirst($comment->user->role) }}</span></span>
                        ...
                        <div class="flex gap-2 mt-3 items-center">
                            ...
                            <!-- Teacher Moderation -->
                            <form method="POST" action="{{ route('teacher.materials.comments.pin', $comment) }}">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-outline" style="font-size: 0.75rem; padding: 4px 8px; {{ $comment->is_pinned ? 'color: #d97706; border-color: #fcd34d; background: #fef3c7;' : '' }}">
                                    {{ $comment->is_pinned ? 'Lepas Semat' : 'Sematkan' }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('teacher.materials.comments.destroy', $comment) }}" onsubmit="return confirm('Hapus komentar ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline" style="font-size: 0.75rem; padding: 4px 8px; color: var(--danger); border-color: #fecaca;">{{ __('Hapus') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="replyOpen" x-transition class="mt-3 pl-12" style="display: none;">
                    <form method="POST" action="{{ route('teacher.materials.comment', $material) }}">
                        @csrf
                                ...
                                <div class="flex gap-3 mb-1">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex gap-2 items-center flex-between">
                                            ...
                                            <!-- Setup Hapus Balasan -->
                                            <form method="POST" action="{{ route('teacher.materials.comments.destroy', $reply) }}" onsubmit="return confirm('Hapus balasan ini?');">
                                                @csrf
                                                @method('DELETE')
        ...
        <form method="POST" action="{{ route('teacher.materials.comment', $material) }}" style="margin-top: 20px;" id="commentForm">
            @csrf
            @error('comment')
...
<script>
    ...
    var quill = new Quill('#editor-container', {
        theme: 'snow',
        placeholder: 'Tambahkan pengumuman atau komentar...',
        modules: {
            toolbar: [
